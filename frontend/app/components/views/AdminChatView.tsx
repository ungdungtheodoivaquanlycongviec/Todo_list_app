...
export default function AdminChatView() {
  ...
  // Load all groups that admin has joined
  ...
  const loadGroupMessages = useCallback(async () => {
    ...
    try {
      ...
      const result = await chatService.getMessages(selectedGroup._id, { limit: 50 });
  ...
  useEffect(() => {
    if (activeContext === 'group') {
      if (!selectedGroup?._id && directConversations.length > 0) {
  ...
  }, [activeContext, selectedGroup?._id, directConversations, activeDirectConversation]);
  ...
  useEffect(() => {
    ...
    const joinRoom = () => {
      ...
      socket.emit('chat:join', selectedGroup._id, (response: any) => {
        if (response.success) {
          loadGroupMessages();
        } else {
          console.error('[AdminChatView] Failed to join chat:', response.error);
        }
      });
    };
    ...
    return () => {
      ...
      if (isConnected && selectedGroup?._id) {
        socket.emit('chat:leave', selectedGroup._id);
      }
    };
  }, [socket, selectedGroup?._id, isConnected, loadGroupMessages, activeContext]);
    ...
    const joinDirectRoom = () => {
      ...
      socket.emit('direct:join', conversationId, (response: any) => {
        if (response?.success) {
          ...
          console.error('[AdminChatView] Failed to join direct conversation:', response?.error);
        }
      });
    };
    ...
    return () => {
      ...
      if (socket && conversationId) {
        socket.emit('direct:leave', conversationId, () => {
          console.log('[AdminChatView] Left direct conversation', conversationId);
        });
      }
    };
  }, [
  ...
  useEffect(() => {
    ...
    const handleNewMessage = (data: { type: string; message: ChatMessage }) => {
      if (activeContext !== 'group' || !selectedGroup?._id) {
        return;
      }
  ...
  useEffect(() => {
    ...
    const handleNotification = (payload: any) => {
      ...
      if (data.contextType === 'group') {
        ...
        if (selectedGroup?._id && data.groupId === selectedGroup._id) {
  ...
  const handleSend = async () => {
    ...
    try {
      ...
      if (pendingAttachment) {
        ...
        try {
          if (activeContext === 'group' && selectedGroup?._id) {
            const attachment = await chatService.uploadAttachment(selectedGroup._id, pendingAttachment);
  ...
  const conversationReady =
    activeContext === 'group' ? Boolean(selectedGroup) : Boolean(activeDirectConversation);

  const conversationTitle =
    activeContext === 'group'
      ? selectedGroup?.name || t('chat.noGroupSelected')
  ...
  return (
    <div className="flex h-full min-h-0 bg-white dark:bg-gray-900">
      <aside className="w-80 border-r border-gray-200 dark:border-gray-800 bg-gray-50/60 dark:bg-gray-900/40 flex flex-col">
        {/* Group Chat Section - Updated to show all groups */}
          ...
          >
            ...
            <div className="flex-1 text-left">
              <p className="text-sm font-medium">
                {selectedGroup ? selectedGroup.name : t('chat.noGroup')}
              </p>
        ...
        {/* Groups list - Only show when group context is active */}
        {activeContext === 'group' && (
                    ...
                    >
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="text-xs text-gray-400 dark:text-gray-500 mt-1">
                          {group.members?.length || 0} {t('chat.members')}
                        </p>
                      </div>
                    </button>
                  );
        ...
        {/* Direct Chat Section - Keep as is */}
        <div className="p-4 border-b border-gray-200 dark:border-gray-800">
                ...
                <>
                  <button
                    onClick={() => {
                      if (activeContext === 'group' && selectedGroup?._id) {
                        setActiveMeeting({
                          meetingId: `group-${selectedGroup._id}-${Date.now()}`,
                          type: 'group',
                          groupId: selectedGroup._id
                        });
                      } else if (activeContext === 'direct' && activeDirectConversation?._id) {
        ...
        <MeetingView
          ...
          title={
            activeMeeting.type === 'group'
              ? selectedGroup?.name || 'Group Meeting'
...
// Message Item Component - Same as ChatView
