...
// --- 3. STATIC LABELS (FALLBACK - English) ---
...
=======
// --- TÓM TẮT VÀ THUỘC TÍNH UI CỦA VAI TRÒ ---
...
    summary: string;
    capabilities: string;
    badgeColor: string; // Tailwind CSS classes
  }
> = {
...
// --- 8. DYNAMIC FUNCTIONS (I18N SUPPORT) ---
...
export const getRoleSummaries = (t: TranslateFunction): Record<GroupRoleKey, { scope: 'full' | 'read_only' | 'folder_scoped'; summary: string; capabilities: string; badgeColor: string }> => {
  ...
  for (const role of Object.values(GROUP_ROLE_KEYS)) {
    ...
    summaries[role] = {
      ...
      capabilities: t(keys.capabilitiesKey),
...
// --- 9. LEGACY STATIC SUMMARIES (FALLBACK - Vietnamese) ---
export const ROLE_SUMMARIES: Record<
  GroupRoleKey,
  {
    scope: 'full' | 'read_only' | 'folder_scoped';
